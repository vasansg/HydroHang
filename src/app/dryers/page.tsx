...
import { DryerQueueModel, Dryer } from '@/lib/types';
import { format } from 'date-fns';
import { Plus, Loader2, Clock, CheckCircle, XCircle, Search, X, ChevronRight, Wind } from 'lucide-react';
import Link from 'next/link';
import { clsx } from 'clsx';

const DURATION_OPTIONS = [30, 45, 60, 90, 120];

function formatRemainingTime(item: DryerQueueModel): string {
...
export default function DryersPage() {
  ...
  const [queue, setQueue] = useState<DryerQueueModel[]>([]);
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const activeInDryer = queue.filter(
        (q) => q.dryerId === dryerId && ['active', 'waiting'].includes(q.status)
      );
      const position = activeInDryer.length + 1;
  ...
  const activeDryerIds = new Set(
    queue.filter((q) => q.status === 'active').map((q) => q.dryerId)
  );
  const availableCount = dryers.filter((d) => !activeDryerIds.has(d.id)).length;
  const busyCount = dryers.filter((d) => activeDryerIds.has(d.id)).length;
  const waitingCount = queue.filter((q) => q.status === 'waiting').length;

  const filteredDryers = dryers
    .filter((d) => {
      if (search && !d.name.toLowerCase().includes(search.toLowerCase())) return false;
      if (showAvailableOnly && activeDryerIds.has(d.id)) return false;
      return true;
    })
    .sort((a, b) => {
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-0">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black tracking-tight">Dryer Slots</h1>
        ...
        <div className="bg-surface border border-white/10 rounded-2xl p-6 space-y-4 mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <StatPill label="Waiting" value={waitingCount} color="text-amber-400" />
          </div>
        ...
        {/* ── JOIN QUEUE FORM ──────────────────────────────────────────── */}
        {showForm && (
          <div className="bg-surface border border-amber-500/30 rounded-2xl p-6 space-y-4 mb-6"
              ...
              {(['active', 'all'] as const).map((f) => (
                <button
                  ...
                  className={clsx(
                    ...
                    filter === f ? 'bg-amber-600 text-white' : 'bg-white/10 text-white/40 hover:bg-white/20'
                  )}
                >
                  {f === 'active' ? 'Active' : 'All'}{' '}
...
function DryerCard({
  ...
  const activeSession = queue.find(
    (q) => q.dryerId === dryer.id && q.status === 'active'
  );
...
function DryerQueueCard({
  item,
  isOwn,
  onComplete,
  onCancel,
}: {
  item: DryerQueueModel;
  isOwn: boolean;
  onComplete: () => void;
  onCancel: () => void;
}) {
    ...
    <div className={clsx(
      ...
      <div className="flex items-center gap-3">
        ...
        <div className={clsx(
          ...
          isOwn
            ? 'bg-amber-600/40 border-amber-500/50 text-amber-300'
